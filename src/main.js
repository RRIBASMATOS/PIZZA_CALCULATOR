...
$('style').onchange=()=>{updateSubs();calc()};$('substyle').onchange=()=>{updateFermPresets();calc()};$('fermPreset').onchange=()=>{applyFermPreset();calc()};$('flourPreset').onchange=()=>{applyFlour();calc()};['detroitCheese','detroitSauce'].forEach(id=>{if($(id))$(id).onchange=calc});$('calc').onclick=calc;initMenu();initToppings();fill();calc();calcTopping();
...
function calc(){
  ...
  const yType=$('yeastType').value;let y=yeastInstant(eq,profile.styleC,flour.w,flour.fn,sugar,honey,oil);let levainPct=0,lf=0,levainHyd=val('levainHyd')/100;
  if(styleKey==='detroit' && eq<=10 && profile.charlieYeastPct && y<profile.charlieYeastPct)y=profile.charlieYeastPct;
  if(yType==='levain'){lf=levainFlourPct(eq,$('acid').value);levainPct=lf*(1+levainHyd);y=0}else{y=convertYeast(y,yType)}
...
function render(d){
  ...
  $('result').innerHTML=`<div class="result-grid"><div class="box"><h3>Receita</h3><table><tr><td>Farinha total</td><td>${g(d.flourG)}</td></tr><tr><td>Água adicionada</td><td>${g(Math.max(0,d.waterG))}</td></tr><tr><td>Sal</td><td>${g(d.saltG)}</td></tr><tr><td>Fermento comercial</td><td>${d.yType==='levain'?'—':g(d.yeastG,2)}</td></tr><tr><td>Levain total</td><td>${d.yType==='levain'?g(d.levainG):'—'}</td></tr><tr><td>Óleo/azeite na massa</td><td>${g(d.oilG)}</td></tr><tr><td>Açúcar</td><td>${g(d.sugarG)}</td></tr><tr><td>Mel</td><td>${g(d.honeyG)}</td></tr>${d.styleKey==='detroit'?`<tr><td>Malte diastático</td><td>${g(d.maltG,1)}</td></tr>`:''}</table></div><div class="box"><h3>Percentuais</h3><table><tr><td>Hidratação efetiva</td><td>${pct(d.hyd)}</td></tr><tr><td>Sal</td><td>${pct(d.salt)}</td></tr><tr><td>Fermento</td><td>${d.yType==='levain'?'levain':pct(d.y)}</td></tr><tr><td>Levain</td><td>${d.yType==='levain'?pct(d.levainG/d.flourG):'—'}</td></tr><tr><td>Óleo na massa</td><td>${pct(d.oil)}</td></tr><tr><td>Açúcar</td><td>${pct(d.sugar)}</td></tr><tr><td>Mel</td><td>${pct(d.honey)}</td></tr>${d.styleKey==='detroit'?`<tr><td>Malte diastático</td><td>${pct(d.malt)}</td></tr><tr><td>Farinha integral</td><td>${pct(d.wholeWheat)}</td></tr>`:''}</table></div><div class="box"><h3>Processo</h3><table><tr><td>Massa total</td><td>${g(d.totalMass)}</td></tr><tr><td>Massa por unidade</td><td>${g(d.per)}</td></tr><tr><td>Horas equivalentes</td><td>${d.eq.toFixed(1)} h</td></tr><tr><td>Fator térmico superfície</td><td>${d.fs.toFixed(2)}</td></tr><tr><td>Assamento estimado</td><td>${d.bake}</td></tr><tr><td>Alvo da superfície</td><td>${rangeText(surfaceTargetRange(d)[0],surfaceTargetRange(d)[1])}</td></tr><tr><td>Config. forno/preparo</td><td>${ovenCapForStyle(d)}°C</td></tr></table></div><div class="box"><h3>Farinha e cocção</h3><table><tr><td>W / P-L</td><td>${d.flour.w} / ${d.flour.pl}</td></tr><tr><td>Abs / FN</td><td>${d.flour.abs}% / ${d.flour.fn}</td></tr><tr><td>Superfície</td><td>${d.surface.name}</td></tr><tr><td>Temperatura máxima do forno</td><td>${d.oven}°C</td></tr></table></div></div>${detroitBlock}<div class="box wide"><h3>Cronograma operacional sugerido</h3><ol>${schedule.map(x=>`<li>${x}</li>`).join('')}</ol></div><div class="box wide"><h3>Modo de preparo recomendado</h3><ol>${prep.map(x=>`<li>${x}</li>`).join('')}</ol></div><div class="box wide"><h3>Justificativa técnica</h3><ul>${tech.map(x=>`<li>${x}</li>`).join('')}</ul></div>${d.warn.length?d.warn.map(x=>`<div class="warn">${x}</div>`).join(''):'<div class="ok">Configuração coerente com o envelope técnico selecionado.</div>'}<div class="warn"><b>Nota:</b> modelos de fermentação e cocção são heurísticos e calibráveis. A parte Detroit agora combina nossa metodologia com os coeficientes da planilha Charlie Anderson.</div>`
}
