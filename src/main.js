...
$('style').onchange=()=>{updateSubs();calc()};$('substyle').onchange=()=>{updateFermPresets();calc()};$('fermPreset').onchange=()=>{applyFermPreset();calc()};$('flourPreset').onchange=()=>{applyFlour();calc()};['detroitCheese','detroitSauce'].forEach(id=>{if($(id))$(id).onchange=calc});$('calc').onclick=calc;initMenu();initToppings();initPrepGuide();initCourseModule();fill();calc();calcTopping();renderPrepGuide();renderCourseModule();
...
function initCourseModule(){
  if(!$('courseView')||!$('courseSheet'))return;
  COURSE_DATA.sheets.forEach((sheet,idx)=>$('courseSheet').add(new Option(sheet.name,idx)));
  $('courseView').onchange=()=>{renderCourseModule()};
  $('courseSheet').onchange=()=>{renderCourseModule()};
  $('courseSearch').addEventListener('input',()=>renderCourseModule());
}
function esc(x){return String(x??'').replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));}
function cellHtml(cell){const value=esc(cell.value||'');if(cell.link){return `<a href="${esc(cell.link)}" target="_blank" rel="noopener noreferrer">${value && value!=='Abrir' && value!=='Fonte'?value:'Abrir link'}</a>`}return value;}
function rowText(cells){return cells.map(c=>`${c.value||''} ${c.link||''}`).join(' ').toLowerCase();}
function lessonText(a){return Object.values(a).join(' ').toLowerCase();}
function renderCourseModule(){
  if(!$('courseResult'))return;
  const q=($('courseSearch')?.value||'').trim().toLowerCase();
  const view=$('courseView')?.value||'course';
  if(view==='sheet')return renderCourseSheet(q);
  const html=[];
  html.push(`<div class="warn"><b>Benchmarking usado:</b> o curso segue a lógica de cursos por etapas, como My Pizza Corner, que divide o aprendizado em mistura, sova, teste de glúten, boleamento, toppings e abertura, e a escola online AVPN, que estrutura aprendizagem teórico-prática. A planilha completa foi incorporada no app, incluindo links reais extraídos dos hiperlinks do Excel.</div>`);
  COURSE_DATA.course.forEach(fase=>{
    const modulos=[];
    fase.modulos.forEach(mod=>{
      const aulas=mod.aulas.filter(a=>!q||lessonText(a).includes(q));
      if(!aulas.length)return;
      const rows=aulas.map(a=>`<tr><td>${esc(a.id)}</td><td>${esc(a.semana)}</td><td>${esc(a.tipo)}</td><td><b>${esc(a.conteudo)}</b><br><span class="hint">${esc(a.resumo)}</span></td><td>${esc(a.fonte)}</td><td>${a.link?`<a href="${esc(a.link)}" target="_blank" rel="noopener noreferrer">Abrir recurso</a>`:'—'}</td><td>${esc(a.exercicio)}</td><td>${esc(a.criterio)}</td></tr>`).join('');
      modulos.push(`<details open class="box wide"><summary><b>${esc(mod.modulo)}</b> — ${aulas.length} aula(s)</summary><div class="table-wrap"><table><thead><tr><th>ID</th><th>Semana</th><th>Tipo</th><th>Conteúdo</th><th>Fonte</th><th>Link</th><th>Exercício prático</th><th>Critério de aprovação</th></tr></thead><tbody>${rows}</tbody></table></div></details>`);
    });
    if(modulos.length)html.push(`<h3>${esc(fase.fase)}</h3>${modulos.join('')}`);
  });
  if(html.length===1)html.push('<div class="warn">Nenhum resultado encontrado para a busca.</div>');
  $('courseResult').innerHTML=html.join('');
}
function renderCourseSheet(q){
  const idx=Number($('courseSheet')?.value||0);
  const sheet=COURSE_DATA.sheets[idx]||COURSE_DATA.sheets[0];
  const headers=sheet.headers.map(h=>`<th>${esc(h)}</th>`).join('');
  const rows=sheet.rows.filter(cells=>!q||rowText(cells).includes(q)).map(cells=>`<tr>${cells.map(c=>`<td>${cellHtml(c)}</td>`).join('')}</tr>`).join('');
  $('courseResult').innerHTML=`<div class="box wide"><h3>Aba: ${esc(sheet.name)}</h3><p class="hint">Exibindo ${rows?sheet.rows.filter(cells=>!q||rowText(cells).includes(q)).length:0} linhas. Links foram extraídos dos hiperlinks originais da planilha, não apenas do texto visível “Abrir”.</p><div class="table-wrap"><table><thead><tr>${headers}</tr></thead><tbody>${rows||'<tr><td colspan="20">Nenhum resultado encontrado.</td></tr>'}</tbody></table></div></div>`;
}
